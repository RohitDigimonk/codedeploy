...
def get_user_story(request):
    ...
    if AR_USER_STORY.objects.filter(backlog_parent=instance_of_backlog).exists():
        ...
        get_userstory = {}
    return render(request, 'admin/iterations/get_user_storyes.html', {'date':datetime.now(),"user_story_data": get_userstory,'selected_user_storyes':selected_user_storyes})
...
def index(request):
    ...
    if get_status_of_permission:
        ...
        return render(request, 'admin/iterations/index.html',{'date':datetime.now(),"Rearrange_Request_msg":Rearrange_Request_msg,'Remove_done_msg':Remove_done_msg,'Remove_Request_msg':Remove_Request_msg,'Not_Remove_msg':Not_Remove_msg,'all_column_list':all_column_list,'get_show_column':get_show_column,'get_all_iterations':get_all_iterations,'user_name': request.session['user_name'], 'BASE_URL': settings.BASE_URL})
    else:
    ...
    if product_view.check_permition(request, 'Manage Iterations', 1):
        ...
        if request.method == 'POST':
            ...
            if IterationForm_get.is_valid():
                ...
                EndDateSet = datetime.strptime(StartDate, "%m/%d/%Y %I:%M %p")
        ...
        return render(request, 'admin/iterations/edit_iteration.html',{'IterationInfo':IterationInfo,'date':datetime.now(),'start_data':start_data,'end_data':end_data,'IterationForm': IterationForm_get, 'user_name': request.session['user_name'],'BASE_URL': settings.BASE_URL})
    else:
        msg = get_object_or_404(Notification, page_name="Authorized", notification_key="Error")
        error_data = msg.notification_desc
        return render(request, 'admin/dashboard/no_permssion.html', {'BASE_URL': settings.BASE_URL,'error_message':error_data})
